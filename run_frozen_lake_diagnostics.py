...
def print_obs(obs, n_states):
    """Print position + directional sensor readings."""
    # Position modality: first n_states channels
    pos_obs = obs[:n_states]
    observed_pos = int(jnp.argmax(pos_obs))
    grid_size = int(n_states ** 0.5)
    r, c = divmod(observed_pos, grid_size)
    print(f"    Position: ({r},{c})")

    # Directional modality: last 4 channels
    parts = []
    for d in range(4):
        val = float(obs[n_states + d])
        if val > 0.5:
            parts.append(f"{DIR_NAMES[d]}=YES")
        else:
            parts.append(f"{DIR_NAMES[d]}=no")
    print(f"    Directional: {' '.join(parts)}")


def print_hole_heatmap(q_static, holes, grid_size):
    """Print expected hole probability per cell (marginal over configs)."""
    n_states = grid_size * grid_size
    hole_prob = np.zeros(n_states)
...
def run_diagnostic_episode(agent, env, args, holes):
...
def main():
    ...
    run_diagnostic_episode(agent, env, args, holes)
